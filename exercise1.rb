...
media[:response][:results].each do |add_key|
	if add_key[:type] == "article"
		add_key[:views] = 0
	end
end

def read_article(media)
	media[:response][:results].sample[:views] += 1

end

def display_views(media)
	media[:response][:results].each do |displays|
		puts displays[:webTitle]
		puts displays[:views]
	end
end
